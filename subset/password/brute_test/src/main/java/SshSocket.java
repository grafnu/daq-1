...
public class SshSocket implements Runnable{
	...
	public SshSocket(String host, Map macDevices,int connectionPort) {
		...
		reportHandler.addText("Manufacturer : " + macDevices.get(macAddress.substring(0,6)) + "*");
			...
			while(!testFinished) {
				...
				if(usernameIndex > jsonUsernames.length - 1 ) {
					...
					attempts++;
					System.out.println("attempt " + (attempts + 1) + " made using: "+ jsonUsernames[usernameIndex] + "/" + jsonPasswords[passwordIndex]);
					try {
						...
						try {
							...
							reportHandler.addText("Successful Login" + ":-" + jsonUsernames[usernameIndex] + " : " +  jsonPasswords[passwordIndex]  + "*");
							...
							reportHandler.addText("Failed Login" + ":-" + jsonUsernames[usernameIndex] + " : " +  jsonPasswords[passwordIndex]  + "*");
							passwordIndex++;
						}
