...
public class AlliedInterrogator implements Runnable{
	...
	String[] expected = {"login:","Password:","Last login:","#","Login incorrect","Connection closed by foreign host."};
	
	String [] command = {"enable","show interface port1.0.","show platform port port1.0.","show run","show stack"};
	
	int interfacePos = 1;
	int platformPos = 2;
	
	String[] show_stack_expected = {"ID","Pending ID","MAC address","Priority","Status","Role","\n"};
	int[] show_stack_pointers = new int[7];
	String[] show_stack_data = new String[6];
	
	String[] show_interface_expected = {"port1","\n",
			                            "Link is ",",",
	...
	String[] show_platform_expected = {"port1","\n",
		                            "enabled:","\n",
		                            "loopback:","\n",
		                            "link:","\n",
		                            "speed:","m",
	...
	public void receiveData(String data){
		if(debug) {
			System.out.println(java.time.LocalTime.now() + "receiveDataLen:" + data.length() + "receiveData:" + data);
		}
	...
	private void parse_packet(String raw_data,String[] show_expected,int[] show_pointers){
	    ...
	    for(int port=0;port<number_switch_ports;port++){
	        ...
	        if(chunk_e == -1)
	            chunk_e = raw_data.length();

	        String temp_data = raw_data.substring(chunk_s,chunk_e);

	        if(debug)
	            System.out.println("length" + temp_data.length() + "temp_data:" + temp_data);

	        if (requestFlag == (interfacePos+1)){
	            parse_single(temp_data,show_interface_expected,show_interface_pointers,show_interface_data);
	        }
					...
					if(!userEnabled) {
	                	...
	                	if(data.indexOf(device_hostname) >= 0 && data.length() < 20){
		                    ...
		                    if(requestFlag < finish){
		                    	telnetClientSocket.writeData(command[requestFlag] + "\n");
		                    	System.out.println("command:" + command[requestFlag] + " request_flag:"+requestFlag);
		                    	requestFlag += 1;
		                    }
		                    else{
		                    ...
		                    switch(requestFlag){
			                    ...
			                    case 5:
			                        ...
			                        for(int x=0;x < show_stack_expected.length;x++){
			                            ...
			                            if(x > 0) {
			                                show_stack_pointers[x-1] = show_stack_pointers[x] - show_stack_pointers[x-1];
			                            }
			                        }
