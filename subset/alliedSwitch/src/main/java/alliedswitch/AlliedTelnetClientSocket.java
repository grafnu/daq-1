...
public class AlliedTelnetClientSocket implements TelnetNotificationHandler, Runnable {
    ...
    public AlliedTelnetClientSocket(String remoteIpAddress, int remotePort, AlliedInterrogator interrogator, boolean debug) {
    ...
    private void addOptionHandlers() {
        ...
        //EchoOptionHandler echoOptionHandler = new EchoOptionHandler(true, false, true, false);//original method
        ...
        try{
            ...
            System.err.println("Error registering option handlers InvalidTelnetOptionException: " + e.getMessage());
        }
        catch (IOException e) {
    		...
    		try {
	    		if(rxQueue.isEmpty()) {
	    			...
	    			if(rxTemp.equals("")) {
    					...
    					System.out.println(java.time.LocalTime.now() + "rxDataLen:"+rxGathered.length()+"rxData:"+rxGathered);
	    				...
	    				if(parseFlag) {
	    					...
	    					System.out.println(java.time.LocalTime.now() + "rxGatheredLen:" + rxGathered.length() + "rxGathered:" + rxGathered);
